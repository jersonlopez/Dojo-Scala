...
// Método para eliminar la mascota indicada de la lista
  def removerMascota(id: Int) = Action {
    ...
    mascotas.find(_.id == id) match {
      case Some(m) =>
        mascotas = mascotas.filter(x => x.id != id)
  ...
  def actualizarMascotaSQL = Action { implicit request =>
    ...
    cuerpoJson.validate[Pet] match {
      ...
      case success: JsSuccess[Pet] =>
        ...
        var nuevaMascota = Pet(success.get.id,
                               success.get.name,
                               success.get.kind,
                               success.get.gender,
        ...
        try {
          ...
          val resultado = query.executeUpdate(
            "UPDATE pet SET name = '" + nuevaMascota.name + "', kind = '" + nuevaMascota.kind + "', gender = '" + nuevaMascota.gender + "', location = '" + nuevaMascota.location + "',state = '" + nuevaMascota.state + "' WHERE id = " + nuevaMascota.id)
        } finally {
      ...
      case e: JsError =>
        BadRequest("No se pudo actualizar porque hay malos parametros!!")
    }
  }
    ...
    cuerpoJson.validate[Pet] match {
      ...
      case success: JsSuccess[Pet] =>
        ...
        try {
          ...
          val query = conexion.createStatement
          val resultado = query.executeUpdate(
            "INSERT INTO pet VALUES(" + nuevaMascota.id + ",'" + nuevaMascota.name + "','" + nuevaMascota.kind + "','" + nuevaMascota.gender + "','" + nuevaMascota.location + "','" + nuevaMascota.state + "')")
          ...
          case e: Exception =>
            Ok(
              "No se puede insertar la mascota porque la clave indicada ya existe o hubo un error.")
        } finally {
    ...
    cuerpoJson.validate[Pet] match {
      ...
      case success: JsSuccess[Pet] =>
        ...
        mascotas.find(_.id == success.get.id) match {
          ...
          case Some(m) =>
            mascotas = mascotas.map(aux =>
              if (aux.id == success.get.id) nuevaMascota else aux)
            Ok("La mascota ha sido actualizada exitosamente!")
    ...
    cuerpoJson.validate[Pet] match {
      ...
      case success: JsSuccess[Pet] =>
        ...
        mascotas.find(_.id == success.get.id) match {
          ...
          case Some(m) =>
            Ok(
              "No se puede insertar la mascota porque la clave indicada ya existe")
